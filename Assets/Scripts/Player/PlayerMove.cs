...
public class PlayerMove : MonoBehaviour {
    ...
    private bool Dont_Moving(Vector3 Moving_Dir) { //계단이 있는지 , 2층에 있어서 움직일 수 없는지 ,높이에 관련된 레이저, 높이 조절함수
        ...
        if (Razer(transform.position, Vector3.down,PlayerManager.Player_Manager_Instance.Block_Size)) //내 아래로 레이저를 쏜다.(라바,계단,블럭 등이 있는지 파악)
        {
